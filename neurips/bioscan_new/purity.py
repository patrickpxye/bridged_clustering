...
def split_family_samples(family_data, supervised=0.05, random_state=42):
    ...
    proportions = {'gene': 0.2, 'supervised': supervised, 'inference': 1.0 - 0.2 - supervised}
...
def perform_clustering_kmeans(image_samples, gene_samples, images, image_model, image_transform, barcode_tokenizer, barcode_model, n_families):
    """
...
def perform_clustering_kmeanscon(image_samples, gene_samples, images, image_model, image_transform, barcode_tokenizer, barcode_model, n_families):
    """
    Perform KMeans clustering on image and gene samples.
    Returns the trained KMeans objects and raw features.
    """
...
def perform_clustering_spectral(image_samples, gene_samples,
                                images, image_model, image_transform,
                                barcode_tokenizer, barcode_model,
                                n_families):
    """
    Perform Spectral Clustering on image and gene samples.
    Returns (image_clusterer, gene_clusterer, image_feats, gene_feats,
             image_labels, gene_labels).
    """
    # 1) Encode
    image_feats = encode_images(image_samples['processid'].values,
                                images, image_model, image_transform)
    gene_feats  = encode_genes(gene_samples['dna_barcode'].values,
                               barcode_tokenizer, barcode_model)

    # 2) Cluster
    image_clusterer = SpectralClustering(
        n_clusters=n_families,
        affinity="nearest_neighbors",
        assign_labels="discretize",
        random_state=42
    )
    image_labels = image_clusterer.fit_predict(image_feats)

    gene_clusterer = SpectralClustering(
        n_clusters=n_families,
        affinity="nearest_neighbors",
        assign_labels="discretize",
        random_state=42
    )
    gene_labels = gene_clusterer.fit_predict(gene_feats)

    return image_clusterer, gene_clusterer, image_feats, gene_feats, image_labels, gene_labels


def perform_clustering_agglomerative(image_samples, gene_samples,
                                     images, image_model, image_transform,
                                     barcode_tokenizer, barcode_model,
                                     n_families):
    """
    Perform Agglomerative (hierarchical) Clustering on image and gene samples.
    """
    # Encode
    image_feats = encode_images(image_samples['processid'].values,
                                images, image_model, image_transform)
    ...
    # Agglomerative clustering (Ward linkage gives compact, roughly equal‐sized clusters)
    image_clusterer = AgglomerativeClustering(
        n_clusters=n_families,
        linkage="ward"
    )
    image_labels = image_clusterer.fit_predict(image_feats)

    gene_clusterer = AgglomerativeClustering(
        n_clusters=n_families,
        linkage="ward"
    )
    gene_labels = gene_clusterer.fit_predict(gene_feats)
...
def perform_clustering_gmm(image_samples, gene_samples,
                           images, image_model, image_transform,
                           barcode_tokenizer, barcode_model,
                           n_families):
    """
    Fit a Gaussian Mixture Model (EM) with fixed n_components.
    """
    # Encode
    image_feats = encode_images(image_samples['processid'].values,
                                images, image_model, image_transform)
    ...
    # Gaussian Mixture
    image_clusterer = GaussianMixture(
        n_components=n_families,
        covariance_type="full",  # or "diag" for speed
        random_state=42
    )
    image_clusterer.fit(image_feats)
    image_labels = image_clusterer.predict(image_feats)

    gene_clusterer = GaussianMixture(
        n_components=n_families,
        covariance_type="full",
        random_state=42
    )
    gene_clusterer.fit(gene_feats)
    gene_labels = gene_clusterer.predict(gene_feats)
...
def decisionVector_simple(sample, morph_column='morph_cluster', gene_column='gene_cluster', dim=5):
...
def decisionVector_hungarian(sample, x_column, y_column, dim=5):
    # build association matrix as before…
    assoc = np.zeros((dim, dim), int)
    for i in range(dim):
        for j in range(dim):
            assoc[i,j] = np.sum((sample[x_column]==i) & (sample[y_column]==j))

    # solve max-weight matching by turning it into a *min*-cost problem
    row_idx, col_idx = linear_sum_assignment(-assoc)
    decision = np.full(dim, -1, dtype=int)
    decision[row_idx] = col_idx
    return decision


def decisionVector_threshold(sample, x_column, y_column, dim=5, threshold=0.6, eps=1e-9):
    assoc = np.zeros((dim, dim), float)
    for i in range(dim):
        for j in range(dim):
            assoc[i,j] = np.sum((sample[x_column]==i) & (sample[y_column]==j))
    probs = assoc / (assoc.sum(axis=1, keepdims=True) + eps)

    # pick best j only if it’s confidently above the threshold
    best_j = np.argmax(probs, axis=1)
    decision = np.where(probs[np.arange(dim), best_j] >= threshold,
                        best_j,
                        -1)
    return decision


def decisionVector_margin(sample, x_column, y_column, dim=5, margin=0.1, eps=1e-9):
    assoc = np.zeros((dim, dim), int)
    for i in range(dim):
        for j in range(dim):
            assoc[i,j] = np.sum((sample[x_column]==i) & (sample[y_column]==j))
    ...
    decision = np.full(dim, -1, dtype=int)
    for i in range(dim):
        top2 = np.argsort(assoc[i])[::-1][:2]
        gap = (assoc[i, top2[0]] - assoc[i, top2[1]]) / (assoc[i, top2[0]] + eps)
        if gap >= margin:
            decision[i] = top2[0]
    return decision
...
def build_decision_matrix(supervised_samples, image_clusters, gene_clusters, n_families, voting='hungarian'):
    """
    ...
    VOTING_FUNCS = {
        'hungarian': decisionVector_hungarian,
        'simple': decisionVector_simple,
        'threshold': decisionVector_threshold,
        'margin': decisionVector_margin,
    }
    
    FN = VOTING_FUNCS.get(voting, None)
    decision_matrix = FN(supervised_samples, morph_column='image_cluster', gene_column='gene_cluster', dim=n_families)
...
###########################
def run_experiment(csv_path, image_folder, n_families, n_samples=50, supervised=0.05, clustering_method='kmeans', voting='hungarian'):
    ...
    # Call the clustering function based on the specified method
    CLUSTERING_FUNCS = {
        'kmeans':           perform_clustering_kmeans,
        'kmeanscon':        perform_clustering_kmeanscon,
        'spectral':         perform_clustering_spectral,
        'agglomerative':    perform_clustering_agglomerative,
        'gmm':              perform_clustering_gmm,
    }

    try:
        fn = CLUSTERING_FUNCS[clustering_method]
    except KeyError:
        raise ValueError(f"Unknown clustering method: {clustering_method}")

    # all of these functions share the same signature,
    # so we can just call fn(...) once:
    image_kmeans, gene_kmeans, _, gene_features, image_clusters, gene_clusters = \
        fn(inference_plus_supervised,gene_plus_supervised,images,image_model,image_transform,barcode_tokenizer,barcode_model,n_families)
    ...
    decision_matrix = build_decision_matrix(supervised_samples, image_clusters, gene_clusters, n_families, voting='simple')
    ...
    # Compute errors
    bkm_error, bkm_mse = evaluate_loss(bkm_predictions, bkm_actuals)
    ...
    return bkm_error, bkm_mse, ami_image, ami_gene, decision_accuracy
...
if __name__ == '__main__':
    ...
    clustering_methods = ['kmeanscon', 'kmeans', 'spectral', 'agglomerative', 'gmm']
    voting_methods = ['hungarian', 'simple', 'threshold', 'margin']
    ...
    mae_matrix = np.empty((len(clustering_methods), len(voting_methods), n_trials))
    mse_matrix = np.empty((len(clustering_methods), len(voting_methods), n_trials))
    ami_image_matrix = np.empty((len(clustering_methods), len(voting_methods), n_trials))
    ami_gene_matrix = np.empty((len(clustering_methods), len(voting_methods), n_trials))
    decision_accuracy_matrix = np.empty((len(clustering_methods), len(voting_methods), n_trials))
    ...
    for trial in range(n_trials):
        n_families = random.choice(n_families_values)
        n_samples = random.choice(n_samples_values)
        supervised = random.choice(supervised_values)
        for clustering_method in clustering_methods:
            for voting_method in voting_methods:
                    print(f"Running experiment: n_families={n_families}, n_samples={n_samples}, supervised={supervised}, clustering={clustering_method}, voting={voting_method}, trial={trial + 1}/{n_trials}")
                    bkm_error, bkm_mse, ami_image, ami_gene, decision_accuracy = run_experiment(
                        csv_path, image_folder, n_families, n_samples,
                        supervised=supervised, clustering_method=clustering_method,
                        voting=voting_method
                    )
                    # Store results in the matrix
                    mae_matrix[clustering_methods.index(clustering_method), voting_methods.index(voting_method), trial] = bkm_error
                    mse_matrix[clustering_methods.index(clustering_method), voting_methods.index(voting_method), trial] = bkm_mse
                    ami_image_matrix[clustering_methods.index(clustering_method), voting_methods.index(voting_method), trial] = ami_image
                    ami_gene_matrix[clustering_methods.index(clustering_method), voting_methods.index(voting_method), trial] = ami_gene
                    decision_accuracy_matrix[clustering_methods.index(clustering_method), voting_methods.index(voting_method), trial] = decision_accuracy
    # save to npy files
    np.save('mae_matrix_{}.npy'.format(experiment_key), mae_matrix)
    np.save('mse_matrix_{}.npy'.format(experiment_key), mse_matrix)
    np.save('ami_image_matrix_{}.npy'.format(experiment_key), ami_image_matrix)
    np.save('ami_gene_matrix_{}.npy'.format(experiment_key), ami_gene_matrix)
    np.save('decision_accuracy_matrix_{}.npy'.format(experiment_key), decision_accuracy_matrix)


    np.save('results/mae_matrix_{}.npy'.format(experiment_key), mae_matrix)
    np.save('results/mse_matrix_{}.npy'.format(experiment_key), mse_matrix)
    np.save('results/ami_image_matrix_{}.npy'.format(experiment_key), ami_image_matrix)
    np.save('results/ami_gene_matrix_{}.npy'.format(experiment_key), ami_gene_matrix)
    np.save('results/decision_accuracy_matrix_{}.npy'.format(experiment_key), decision_accuracy_matrix)