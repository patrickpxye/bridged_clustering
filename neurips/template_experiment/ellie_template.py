...
def run_experiment(df, image_folder, supervised_fraction, trial_num=0, supervised_value=0.01):
    ...
    # Evaluate
    # mean_teacher_error = evaluate_loss(mt_preds, mt_actuals)
...
def run_all_trials(df, image_folder):
    ...
    for s in SUPERVISED_VALUES:
        ...
        for trial in range(N_TRIALS):
            ...
            trial_results = run_experiment(df.copy(), image_folder, s, trial_num=trial, supervised_value=s)
...
if __name__ == '__main__':
    ...
    # method_names = [
    #     'BKM', 'KNN', 'Mean Teacher', 'XGBoost',
    #     'Laplacian RLS', 'TSVR', 'TNNR', 'UCVME', 'RankUp', 'AGDN'
    # ]

    # for s in SUPERVISED_VALUES:
    #     print(f"\nSupervised {s:.2%}")
    #     for method in method_names:
    #         mean_score = np.mean(results[s][method])
    #         print(f"{method} MAE: {mean_score:.2f}")
