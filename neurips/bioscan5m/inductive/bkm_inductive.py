...
def run_experiment(csv_path, image_folder, n_families, n_samples=50, supervised=0.05):
    ...
    unlabeled_loader = torch.utils.data.DataLoader(
        list(zip(
            torch.tensor(image_samples['morph_coordinates'].tolist(), dtype=torch.float32),
            torch.zeros_like(torch.tensor(image_samples['morph_coordinates'].tolist(), dtype=torch.float32))
        )),
        batch_size=32, shuffle=True
    )
    ...
    return np.mean(bkm_error), np.mean(knn_error), np.mean(mean_teacher_error), np.mean(gcn_error), np.mean(agdn_error)
...
if __name__ == '__main__':
    ...
    results_matrix = np.empty((1, 5, 4, 5))
    total_results_matrix = np.empty((1, 5, 4, 5, 20))
    ...
    for n_families_idx, n_families in enumerate(n_families_values):
        for n_samples_idx, n_samples in enumerate(n_samples_values):
            for supervised_idx, supervised in enumerate(supervised_values):
                bkm_total, gcn_total, agdn_total, mt_total, knn_total = [], [], [], [], []
                success = 0
                for i in range(n_trials):
                    print(f"Experiment {i+1} for n_families={n_families}, n_samples={n_samples}, supervised={supervised}:")
                    bkm, knn, mt, gcn, agdn = run_experiment(csv_path, image_folder, n_families=n_families, n_samples=n_samples, supervised=supervised)
                    if bkm < gcn and bkm < agdn and bkm < mt and bkm < knn:
                        success += 1
