...
def simgleExperiment(super_size, n_families, n_neighbors, test_size=20):
    ...
    return result_bkm, [], result_lin

    return result_bkm, result_knn, result_lin


def addExperiment(super_size, n_families, n_neighbors, test_size=20):
    ...
    # train clustering models; formalize data as coordinates; predict clusters for train set, only useful for the fully supervised portion
    morph_df, morph_kmeans = fitMorphClusters(train_morph_df, n_families, full_feat=True)
    gene_df, gene_kmeans = fitGeneClusters(train_gene_df, n_families, full_feat=False)

    # predict cluster assignment for test set, only useful for the bridged clustering method
    test_morph_df = predictMorphClusters(test_morph_df, morph_kmeans, full_feat=True)
    test_gene_df = predictGeneClusters(test_gene_df, gene_kmeans, full_feat=False)
...
result_bkm, result_lin = [], []
result_knn_1, result_knn_2, result_knn_3 = [], [], []
...
for i in range(500):
    for super_size in [samplesize[2]]:
        ...
        result_knn_1.extend(knn)
        _, knn, _ = addExperiment(super_size,n_families,2)
        result_knn_2.extend(knn)
        _, knn, _ = addExperiment(super_size,n_families,3)
        result_knn_3.extend(knn)

plotDensityGraph(result_bkm, result_knn_1, result_knn_2, result_knn_3, result_lin)